...
public class TenantIdentifierListener extends DefaultVisitListener
{
	...
	<E> void pushConditions(VisitContext context, Table<?> table, Field<E> field, E values) {
		...
		if (context.queryPart() == table) {
			...
			// FROM clause
			if (clauses.contains(Clause.SELECT_FROM) ||
				clauses.contains(Clause.UPDATE_UPDATE) ||
				clauses.contains(Clause.DELETE_DELETE)) {

				// If we're declaring a TABLE_ALIAS... (e.g. "T_BOOK" as "b")
				if (clauses.contains(Clause.TABLE_ALIAS)) {
	...
	List<Clause> clauses(VisitContext context) {
		...
		int index = result.lastIndexOf(Clause.SELECT);
	...
	public void clauseStart(VisitContext context) {
		...
		if (context.clause() == Clause.SELECT ||
			context.clause() == Clause.UPDATE ||
			context.clause() == Clause.DELETE ||
			context.clause() == Clause.INSERT) {
			push(context);
		}
	}
	...
	public void clauseEnd(VisitContext context) {
		...
		if (context.clause() == Clause.SELECT_WHERE ||
			context.clause() == Clause.UPDATE_WHERE ||
			context.clause() == Clause.DELETE_WHERE) {
	...
	public void visitEnd(VisitContext context) {
		...
		if (context.queryPart() instanceof Condition) {
			...
			if (clauses.contains(Clause.SELECT_WHERE) ||
				clauses.contains(Clause.UPDATE_WHERE) ||
				clauses.contains(Clause.DELETE_WHERE)) {
				where(context, true);
			}
		}
	}
